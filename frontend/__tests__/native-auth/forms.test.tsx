...
describe("native account recipient forms", () => {
  ...
  it("scrubs a fragment, retains it through password login, then accepts exact membership before TOS", async () => {
    ...
    await screen.findByText("AUTH$EXISTING_ACCOUNT");
    await waitFor(() => expect(screen.queryByRole("status")).not.toBeInTheDocument());
    expect(screen.queryByLabelText("AUTH$NEW_PASSWORD")).not.toBeInTheDocument();
    fireEvent.change(screen.getByLabelText("AUTH$PASSWORD"), { target: { value: "existing-password" } });
  ...
  it("validates password confirmation and completes new-account setup", async () => {
    ...
    fireEvent.change(await screen.findByLabelText("AUTH$NEW_PASSWORD"), { target: { value: "a unique long password" } });
    fireEvent.change(screen.getByLabelText("AUTH$CONFIRM_PASSWORD"), { target: { value: "a different long password" } });
    fireEvent.submit(containingForm(screen.getByRole("button", { name: "AUTH$SET_UP_ACCOUNT" })));
    expect(await screen.findByRole("alert")).toHaveTextContent("AUTH$PASSWORD_MISMATCH");
    expect(enroll).not.toHaveBeenCalled();
    fireEvent.change(screen.getByLabelText("AUTH$CONFIRM_PASSWORD"), { target: { value: "a unique long password" } });
    fireEvent.submit(containingForm(screen.getByRole("button", { name: "AUTH$SET_UP_ACCOUNT" })));
    await screen.findByText("TOS destination");
  ...
  it("gives expired-link reissue guidance without leaking tokens", async () => {
    ...
    mount("/account-setup", AccountSetup);
    await screen.findByText("AUTH$LINK_UNAVAILABLE");
    expect(window.location.hash).toBe("");
    expect(screen.queryByLabelText("AUTH$NEW_PASSWORD")).not.toBeInTheDocument();
  ...
  it("masks show-once links from PostHog and supports manual copy fallback", async () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "AUTH$COPY_LINK" }));
    await screen.findByText("AUTH$COPY_MANUALLY");
