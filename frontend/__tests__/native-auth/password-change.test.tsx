...
import { NativeAuthError, NativeAuthService } from "#/api/native-auth-service/native-auth-service.api";
...
describe("password change", () => {
  ...
  it("reports an incorrect current password without opening another login form", async () => {
    vi.spyOn(NativeAuthService, "profile").mockResolvedValue({ id: "member", email: "member@example.com", has_password: true, global_permissions: [] });
    vi.spyOn(NativeAuthService, "changePassword").mockRejectedValue(new NativeAuthError(401, "Invalid current password"));
    const login = vi.spyOn(NativeAuthService, "login");
    mount();
    fireEvent.change(await screen.findByLabelText("AUTH$CURRENT_PASSWORD"), { target: { value: "incorrect-password" } });
    fireEvent.change(screen.getByLabelText("AUTH$NEW_PASSWORD"), { target: { value: "new-private-password" } });
    fireEvent.change(screen.getByLabelText("AUTH$CONFIRM_PASSWORD"), { target: { value: "new-private-password" } });
    ...
    expect(screen.getByLabelText("AUTH$CURRENT_PASSWORD")).toHaveValue("");
