...
describe("UserContextMenu", () => {
  ...
  it("does not show the organization preview entry for self-hosted users without create permission", async () => {
    vi.spyOn(OptionService, "getConfig").mockResolvedValue(
      createMockWebClientConfig({
        app_mode: "saas",
        feature_flags: {
    ...
    expect(
      screen.queryByText("ORG$CREATE_ORGANIZATION"),
    ).not.toBeInTheDocument();
    expect(onOpenPreviewModalMock).not.toHaveBeenCalled();
    expect(onOpenCreateOrganizationModalMock).not.toHaveBeenCalled();
  });

  it("keeps real organization creation available in self-hosted mode with create permission", async () => {
    vi.spyOn(OptionService, "getConfig").mockResolvedValue(
      createMockWebClientConfig({
        app_mode: "saas",
        feature_flags: {
