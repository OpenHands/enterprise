...
interface FreeModelsState {
  /**
   * DB-driven set of free ``provider/model`` ids (the same channel that
   * carries `verified`). Empty on backends without free metadata, so
   * consumers uniformly treat "not in set" as paid.
   */
...
interface FreeModelsActions {
   ...
   * ({@link useHydrateFreeModels}) after the backend model list is fetched,
   * so leaf components can read the flags synchronously via zustand
   * selectors without needing a QueryClientProvider in scope.
   */
  setFlags: (flags: {
    freeModels: FreeModelSet;
    defaultModel: string | null;
