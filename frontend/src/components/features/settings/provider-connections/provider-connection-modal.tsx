 ...
 * form, so a blank key on edit is simply omitted from the request — the backend
 * rejects `api_key: null`, so we never send it.
 */
export function ProviderConnectionModal({
  ...
  const updateConnection = useUpdateProviderConnection(orgId);
