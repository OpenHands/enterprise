...
import { useDeleteOrgLlmProfile } from "#/hooks/mutation/use-org-llm-profile-mutations";
import { mutateWithToast } from "#/utils/mutate-with-toast";
import { extractErrorMessage } from "#/utils/extract-error-message";
import { I18nKey } from "#/i18n/declaration";
import { Typography } from "#/ui/typography";

interface OrgDeleteProfileModalProps {
  orgId: string;
  profile: LlmProfileSummary | null;
  onClose: () => void;
}

export function OrgDeleteProfileModal({
  orgId,
  profile,
  onClose,
}: OrgDeleteProfileModalProps) {
  const { t } = useTranslation();
  const deleteProfile = useDeleteOrgLlmProfile(orgId);
