...
export default function PasswordReset(): React.JSX.Element {
  ...
  return (
    <AuthFrame title={t("AUTH$RESET_PASSWORD")}>
      <AuthError message={error} />
      {complete ? (
        <p role="status">{t("AUTH$PASSWORD_RESET_COMPLETE")}</p>
      ) : (
        <>
          <p>{t("AUTH$LINK_UNAVAILABLE")}</p>
          {token &&
            authentication.accountActions.includes("reset_password") && (
              <form
