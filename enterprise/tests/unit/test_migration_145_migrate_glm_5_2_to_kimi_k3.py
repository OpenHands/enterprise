...
MIGRATION_PATH = (
    ...
    / 'migrations'
    / 'versions'
    / '145_migrate_glm_5_2_to_kimi_k3.py'
)
spec = spec_from_file_location('migration_145', MIGRATION_PATH)
assert spec is not None and spec.loader is not None
migration_145 = module_from_spec(spec)
spec.loader.exec_module(migration_145)


def test_replacement_set_is_exact_and_managed_only():
    assert migration_145.MODEL_REPLACEMENTS == {
        'openhands/glm-5.2': 'openhands/kimi-k3',
        'litellm_proxy/glm-5.2': 'litellm_proxy/kimi-k3',
    }
    assert 'anthropic/glm-5.2' not in migration_145.MODEL_REPLACEMENTS
    assert 'custom/glm-5.2' not in migration_145.MODEL_REPLACEMENTS
    # A bare name carries no provenance, so it is treated as BYOK, not managed.
    assert 'glm-5.2' not in migration_145.MODEL_REPLACEMENTS


def test_statement_updates_nested_llm_model_by_exact_match():
    statement = migration_145._replace_llm_model_statement(
        'org_member',
        'agent_settings_diff',
    )
...
def test_upgrade_updates_each_settings_column_for_each_replacement(monkeypatch):
    ...
    migration_145.upgrade()

    assert len(calls) == (
        len(migration_145.JSON_MODEL_COLUMNS) * len(migration_145.MODEL_REPLACEMENTS)
    )
    assert [params for _, params in calls[:2]] == [
        {'old_model': 'openhands/glm-5.2', 'new_model': 'openhands/kimi-k3'},
        {
            'old_model': 'litellm_proxy/glm-5.2',
            'new_model': 'litellm_proxy/kimi-k3',
        },
    ]
...
def test_replace_model_values_updates_nested_profile_models_only_on_exact_match():
    payload = {
        'profiles': {
            ...
            'bare': {'model': 'glm-5.2'},
            'custom': {'model': 'anthropic/glm-5.2'},
        },
        'active': 'managed',
    }

    replaced, changed = migration_145._replace_model_values(payload)
    ...
    assert payload['profiles']['managed']['model'] == 'openhands/glm-5.2'
...
def test_upgrade_encrypted_llm_profiles_reencrypts_changed_rows(monkeypatch):
    ...
    monkeypatch.setattr(migration_145, '_decrypt_json', fake_decrypt)
    monkeypatch.setattr(migration_145, '_encrypt_json', fake_encrypt)

    updated = migration_145._upgrade_encrypted_llm_profile_column(
        Bind(),
        'user',
        'id',
        'llm_profiles',
...
def test_is_saas_web_host_accepts_managed_deployments(web_host):
    assert migration_145._is_saas_web_host(web_host)
...
def test_is_saas_web_host_rejects_other_deployments(web_host):
    assert not migration_145._is_saas_web_host(web_host)
