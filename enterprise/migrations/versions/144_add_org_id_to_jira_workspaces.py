...
def upgrade() -> None:
    op.add_column('jira_workspaces', sa.Column('org_id', sa.UUID(), nullable=True))
    op.execute(
        sa.text(
            """
            UPDATE jira_workspaces AS j
...
def downgrade() -> None:
    ...
    op.drop_column('jira_workspaces', 'org_id')
