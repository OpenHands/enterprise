"""Password reset, replacement, and offline recovery preserve revocation."""
...
from storage.native_auth import BrowserSession, PasswordCredential
...
async def test_reset_single_use_revokes_sessions_without_reenable(
    ...
    admin = await service.auth.login('admin@example.test', PASSWORD, client_ip='1')
...
async def test_reset_requires_recent_same_admin_and_purpose(
    ...
    invitation = await enrollment.issue_invitation(admin_id, 'setup@example.test')
    with pytest.raises(NativeAuthError, match='Invalid or expired'):
        await service.complete_password_reset(
...
async def test_password_change_rotates_all_sessions(native: NativeFixture) -> None:
    enrollment, _ = native
    service = NativePasswordService(enrollment.sessions)
    first = await service.auth.login('admin@example.test', PASSWORD, client_ip='1')
    second = await service.auth.login('admin@example.test', PASSWORD, client_ip='1')
    changed = await service.change_password(
        first.principal.account_id, first.token, PASSWORD, NEW_PASSWORD, client_ip='1'
    )
    assert await service.auth.authenticate_session(first.token) is None
    assert await service.auth.authenticate_session(second.token) is None
    assert await service.auth.authenticate_session(changed.token) is not None
    assert (
        await service.auth.login('admin@example.test', NEW_PASSWORD, client_ip='1')
...
async def test_recent_auth_and_absolute_expiry_cannot_be_extended(
    ...
    enrollment, admin_id = native
    service = NativePasswordService(enrollment.sessions)
    login = await service.auth.login('admin@example.test', PASSWORD, client_ip='1')
    async with configured() as session, session.begin():
    ...
    assert await service.auth.authenticate_session(login.token) is None
...
async def test_concurrent_reset_consumption_revokes_version_once(
    ...
    admin = await service.auth.login('admin@example.test', PASSWORD, client_ip='1')
    other = await enroll(enrollment, admin_id)
    ...
    async with configured() as session:
        assert (
            present(
                await session.get(PasswordCredential, other.principal.account_id)
            ).credential_version
            == 2
        )
...
async def test_recovery_routes_preserve_status_cookie_and_redaction(
    ...
    async with httpx.AsyncClient(
        ...
        assert (
            ...
            and 'openhands_session=' in changed.headers['set-cookie']
        )
