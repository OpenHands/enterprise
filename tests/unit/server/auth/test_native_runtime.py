...
from server.auth import auth_config, keycloak_manager, saas_user_auth
...
async def native_runtime(
    ...
    monkeypatch.setenv('NATIVE_AUTH_APP_ORIGIN', ORIGIN)
    ...
    for module in (user_store, api_key_store, role_store, auth):
    ...
    async with httpx.AsyncClient(
        ...
        response = await client.post(
            ...
            headers={
                **headers,
                'Origin': ORIGIN,
                'X-CSRF-Token': csrf_for_token(login.token),
            },
        )
...
async def test_native_api_key_cookie_needs_csrf_and_trusted_origin(
    ...
    service, _, api_key = native_runtime
    csrf, anonymous = await service.issue_csrf(None)
    async with httpx.AsyncClient(
        transport=httpx.ASGITransport(app=native_app()),
        base_url=ORIGIN,
        cookies={'api_key': api_key, 'openhands_csrf': present(anonymous)},
    ) as client:
...
async def test_native_ineligible_account_blocks_api_and_background_context(
    ...
    with pytest.raises(saas_user_auth.BearerTokenError):
...
async def test_native_authenticated_nonstandard_mutations_require_csrf(
    ...
    async with httpx.AsyncClient(
        ...
        accepted = await client.post(
            path, headers={**headers, 'X-CSRF-Token': csrf_for_token(login.token)}
        )
...
async def test_native_jira_managers_resolve_local_account_and_reject_disabled(
    ...
    assert linked_user is linked and isinstance(auth, SaasUserAuth)
...
async def test_native_incompatible_adapter_fails_closed(
    ...
    monkeypatch.setattr(middleware, 'ENABLE_KEYCLOAK', False)

    async def configured_auth(request: Request) -> UserAuth:
        return DefaultUserAuth()

    monkeypatch.setattr(middleware, 'get_user_auth', configured_auth)
    ...
    response = await middleware.SetAuthCookieMiddleware()(request, call_next)
    assert response.status_code == 401
