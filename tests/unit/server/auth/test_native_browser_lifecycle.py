...
@pytest.mark.asyncio
@pytest.mark.parametrize('operation', ['login', 'enrollment'])
@pytest.mark.parametrize('stale_key_valid', [True, False])
async def test_native_sign_in_replaces_previous_api_key_cookie_identity(
    ...
    invitation = await service.issue_invitation(previous.principal.account_id, email)
    token = invitation['invite_url'].split('#token=', 1)[1]
    if operation == 'login':
        await service.complete_enrollment(token, PASSWORD, client_ip='127.0.0.1')
        path = '/api/auth/password/login'
        body = {'email': email, 'password': PASSWORD}
    else:
        path = '/api/auth/enrollment/complete'
    ...
    async with httpx.AsyncClient(
        ...
        response = await client.post(
            path,
            json=body,
            headers={'Origin': ORIGIN, 'X-CSRF-Token': csrf_for_token(previous.token)},
        )
        assert response.status_code == 200
        assert 'api_key' not in list(client.cookies)
        ...
        assert await service.authenticate_session(previous.token) is None
...
@pytest.mark.asyncio
async def test_password_change_clears_api_key_cookie_while_rotating_session(
    native_runtime: NativeRuntime,
    browser_app: FastAPI,
    async_session_maker: SessionFactory,
) -> None:
    ...
    async with httpx.AsyncClient(
        ...
        set_browser_cookie(client, 'openhands_session', previous.token)
        set_browser_cookie(client, 'api_key', api_key)
        response = await client.post(
            '/api/auth/password/change',
            json={
                'current_password': PASSWORD,
                'new_password': 'A replacement native browser phrase 956',
            },
            headers={'Origin': ORIGIN, 'X-CSRF-Token': csrf_for_token(previous.token)},
        )
        ...
        assert present(current).account_id == previous.principal.account_id
        await assert_key_preserved(
            client, api_key, present(current).account_id, async_session_maker
        )
...
async def test_native_session_identity_wins_over_residual_api_key_cookie(
    ...
    invitation = await service.issue_invitation(
        previous.principal.account_id, 'second@example.com'
    )
    current = await service.complete_enrollment(
        invitation['invite_url'].split('#token=', 1)[1], PASSWORD, client_ip='127.0.0.1'
    )
...
@pytest.mark.asyncio
@pytest.mark.parametrize('account_kind', ['bootstrap', 'invited', 'restored'])
async def test_native_profiles_can_list_personal_organization(
    ...
    if account_kind != 'bootstrap':
        invitation = await service.issue_invitation(
            login.principal.account_id, 'second@example.com'
        )
        login = present(
            await service.complete_enrollment(
                invitation['invite_url'].split('#token=', 1)[1],
                PASSWORD,
                client_ip='127.0.0.1',
            )
        )
