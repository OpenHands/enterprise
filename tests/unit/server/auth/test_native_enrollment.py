...
async def test_enrollment_is_single_use_and_does_not_grant_admin(
    ...
    link = await service.issue_invitation(admin_id, 'Invited@Example.com')
...
async def test_competing_links_resolve_durable_account_without_password_overwrite(
    ...
    service, admin_id = native
    first = await service.issue_invitation(admin_id, 'same@example.com')
    second = await service.issue_invitation(admin_id, 'SAME@example.com')
    ...
    again = await service.auth.login('same@example.com', PASSWORD, client_ip='1')
    assert (
...
async def test_creator_demotion_invalidates_invitation(
    ...
    link = await service.issue_invitation(admin_id, 'another@example.com')
    async with configured() as session, session.begin():
...
async def test_reissue_invalidates_old_link_and_lists_never_expose_secret(
    ...
    service, admin_id = native
    first = await service.issue_invitation(admin_id, 'reissued@example.com')
...
async def test_scoped_invitation_rechecks_and_adds_only_authorized_membership(
    ...
    invitation = await service.issue_invitation(
        admin_id, 'scoped@example.com', admin_id, role_id
    )
    ...
    with pytest.raises(NativeAuthError, match='Global user management'):
        await service.issue_invitation(
            present(result).principal.account_id, 'other@example.com'
        )
...
async def test_concurrent_consumption_creates_one_account(
    ...
    service, admin_id = native
    token = link_token(await service.issue_invitation(admin_id, 'race@example.com'))
        ...
        credentials = (
            await session.scalars(
                select(PasswordCredential).where(
                    PasswordCredential.normalized_login_email == 'race@example.com',
                )
            )
        ).all()
...
async def test_terminal_deletion_revokes_all_earlier_email_links(
    ...
    service, admin_id = native
    first = await service.issue_invitation(admin_id, 'deleted@example.com')
    pending = await service.issue_invitation(admin_id, 'deleted@example.com')
    login = present(
        await service.complete_enrollment(
            link_token(first),
            PASSWORD,
...
async def test_account_metadata_and_expired_invitation_cleanup(
    ...
    service, admin_id = native
    person = await enroll(service, admin_id)
    pending = await service.issue_invitation(admin_id, 'person@example.com')
