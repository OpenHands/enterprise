...
def test_native_app_import_never_constructs_keycloak(mode: str, tmp_path: Path) -> None:
    env = {
        name: value
    ...
    env.update(
        ENABLE_KEYCLOAK=mode,
        OH_WEB_URL='https://native.example.com',
        FRONTEND_DIRECTORY=str(tmp_path),
    )
    script = """
...
with patch('keycloak.keycloak_admin.KeycloakAdmin', side_effect=AssertionError('Keycloak admin initialized')), patch('keycloak.keycloak_openid.KeycloakOpenID', side_effect=AssertionError('Keycloak OIDC initialized')):
    ...
    assert type(get_global_config().lifespan) is SaasAppLifespanService
