...
def grant_manage_users():
    ...
    with (
        patch(
            'server.routes.admin_users.get_user_auth',
            AsyncMock(
                return_value=MagicMock(
                    principal=Principal(
                        uuid.UUID(CALLER_USER_ID), 'password', datetime.now(UTC)
                    )
                )
            ),
        ),
...
async def test_lifecycle_success(mock_app, grant_manage_users, action, method, path):
    ...
    with patch(
        'server.routes.admin_users.EnterpriseUserManagementService.lifecycle',
    ...
    assert lifecycle.await_args.args == (
        action.removesuffix('_user'),
        uuid.UUID(target),
    )
