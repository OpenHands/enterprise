...
class TestSlackV1CallbackProcessor:
    ...
    async def test_budget_exceeded_personal_workspace_shows_credits_message(
        ...
        # Verify credits message was posted to Slack (personal workspace)
        ...
        assert 'OpenHands credits' in posted_message  # Personal workspace message
    ...
    async def test_budget_exceeded_org_workspace_shows_org_budget_message(
        ...
        # Verify org budget message was posted to Slack (multi-user org)
        mock_slack_client.chat_postMessage.assert_called_once()
        call_kwargs = mock_slack_client.chat_postMessage.call_args[1]
        posted_message = call_kwargs.get('markdown_text', '')
        assert 'OpenHands encountered an error' in posted_message
        assert 'budget' in posted_message.lower()  # Org budget message
        assert 'exceeded' in posted_message.lower()
        # Should NOT contain the raw error message
        assert 'litellm.BadRequestError' not in posted_message
