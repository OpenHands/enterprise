...
MIGRATION_PATH = (
    ...
    / 'migrations'
    / 'versions'
    / '159_migrate_deepseek_v4_flash_to_deepseek_v4_1_flash.py'
)
...
def test_replacement_set_is_exact_and_managed_only():
    assert migration_159.MODEL_REPLACEMENTS == {
        'openhands/deepseek-v4-flash': 'openhands/deepseek-v4.1-flash',
        'litellm_proxy/deepseek-v4-flash': 'litellm_proxy/deepseek-v4.1-flash',
    }
    assert 'anthropic/deepseek-v4-flash' not in migration_159.MODEL_REPLACEMENTS
    assert 'custom/deepseek-v4-flash' not in migration_159.MODEL_REPLACEMENTS
    # A bare name carries no provenance, so it is treated as BYOK, not managed.
    assert 'deepseek-v4-flash' not in migration_159.MODEL_REPLACEMENTS


def test_statement_updates_nested_llm_model_by_exact_match():
    statement = migration_159._replace_llm_model_statement(
        'org_member',
        'agent_settings_diff',
    )
...
def test_upgrade_updates_each_settings_column_for_each_replacement(monkeypatch):
    ...
    assert len(calls) == (
        len(migration_159.JSON_MODEL_COLUMNS) * len(migration_159.MODEL_REPLACEMENTS)
    )
    assert [params for _, params in calls[:2]] == [
        {
            'old_model': 'openhands/deepseek-v4-flash',
            'new_model': 'openhands/deepseek-v4.1-flash',
        },
        {
            'old_model': 'litellm_proxy/deepseek-v4-flash',
            'new_model': 'litellm_proxy/deepseek-v4.1-flash',
        },
    ]
...
def test_replace_model_values_updates_nested_profile_models_only_on_exact_match():
    payload = {
        'profiles': {
            'managed': {'model': 'openhands/deepseek-v4-flash'},
            'legacy_proxy': {'model': 'litellm_proxy/deepseek-v4-flash'},
            'bare': {'model': 'deepseek-v4-flash'},
            'custom': {'model': 'anthropic/deepseek-v4-flash'},
        },
        'active': 'managed',
    }

    replaced, changed = migration_159._replace_model_values(payload)

    assert changed is True
    assert replaced['profiles']['managed']['model'] == 'openhands/deepseek-v4.1-flash'
    assert (
    ...
    assert replaced['profiles']['custom']['model'] == 'anthropic/deepseek-v4-flash'
...
def test_upgrade_encrypted_llm_profiles_reencrypts_changed_rows(monkeypatch):
    ...
    monkeypatch.setattr(migration_159, '_decrypt_json', fake_decrypt)
    monkeypatch.setattr(migration_159, '_encrypt_json', fake_encrypt)

    updated = migration_159._upgrade_encrypted_llm_profile_column(
        Bind(),
        'user',
        'id',
        'llm_profiles',
    ...
    assert encrypted_payloads == [
        {
            'profiles': {
                'Default': {'model': 'openhands/deepseek-v4.1-flash'},
            },
            'active': 'Default',
        }
    ]
...
def test_is_saas_web_host_accepts_managed_deployments(web_host):
    assert migration_159._is_saas_web_host(web_host)
...
def test_is_saas_web_host_rejects_other_deployments(web_host):
    assert not migration_159._is_saas_web_host(web_host)
