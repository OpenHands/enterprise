"""Migrate OpenHands DeepSeek V4 Flash settings to DeepSeek V4.1 Flash."""
...
# Only prefixed values are rewritten. A bare ``deepseek-v4-flash`` is not written
# by the managed default path, so such rows are BYOK configs pointed at a
# third-party base_url; rewriting them would aim a DeepSeek key at a V4.1 model.
MODEL_REPLACEMENTS = {
...
# the managed LiteLLM proxy actually serves ``deepseek-v4.1-flash``; a self-hosted
# install runs the same image and chart against its own proxy, and neither the
# model string nor ``base_url`` distinguishes it (``LITE_LLM_API_URL`` defaults
# to the managed URL). ``WEB_HOST`` is the one value that differs per deployment.
...
def upgrade() -> None:
    ...
    if not _is_saas_web_host(web_host):
        print(
            f'Skipping migration 159: WEB_HOST {web_host!r} is not an '
