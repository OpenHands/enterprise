...
def _time(name: str, nullable: bool = False) -> sa.Column[datetime]:
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        _time('created_at'),
        sa.Column('id', sa.UUID(), unique=True, nullable=False),
    )
    op.create_index('ix_browser_session_created_at', 'browser_session', ['created_at'])
    op.create_table(
        'auth_installation',
