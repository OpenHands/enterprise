"""Migrate OpenHands GLM 5.2 settings to Kimi K3."""
...
# Only prefixed values are rewritten. A bare ``glm-5.2`` is not written by the
# managed default path, so such rows are BYOK configs pointed at a third-party
# base_url; rewriting them would aim a GLM key at a Kimi model.
MODEL_REPLACEMENTS = {
...
# the managed LiteLLM proxy actually serves ``kimi-k3``; a self-hosted install
...
def upgrade() -> None:
    ...
    if not _is_saas_web_host(web_host):
        print(
            f'Skipping migration 145: WEB_HOST {web_host!r} is not an '
