"""Migrate OpenHands Kimi K3 settings to DeepSeek V4 Flash."""
...
# Only prefixed values are rewritten. A bare ``kimi-k3`` is not written by the
# managed default path, so such rows are BYOK configs pointed at a third-party
# base_url; rewriting them would aim a Kimi key at a DeepSeek model.
MODEL_REPLACEMENTS = {
...
# the managed LiteLLM proxy actually serves ``deepseek-v4-flash``; a self-hosted install
...
def upgrade() -> None:
    ...
    if not _is_saas_web_host(web_host):
        print(
            f'Skipping migration 153: WEB_HOST {web_host!r} is not an '
