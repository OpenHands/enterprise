...
(in the `openhands` directory), the enterprise server (in the `enterprise` directory), and a React frontend
...
**uv is the only Python package manager.** There is one project at the repo root:
`pyproject.toml` + `uv.lock` cover the `openhands` package *and* the enterprise server.
Run everything from the repo root (`uv run pytest ...`, `uv run pre-commit ...`); there is
no separate environment under `enterprise/`. uv >= 0.11 is required — older versions
silently drop the `[tool.uv]` table in `pyproject.toml`.
...
`uv.lock` at the repo root is the only lockfile. Regenerate it with the same uv version CI
and the Docker build use, so updates only contain actual dependency changes rather than
tool-version migration artifacts:

```bash
UV_VERSION=$(grep -m1 'ARG UV_VERSION=' containers/app/Dockerfile | cut -d= -f2)
uvx uv@$UV_VERSION lock
```

That version is pinned in four places that must stay in sync — bump them together:

```bash
grep -rn 'UV_VERSION\|version: "0\.' containers/app/Dockerfile containers/dev/Dockerfile \
  .github/workflows/py-tests.yml .github/workflows/enterprise-check-migrations.yml
```

Note that `[tool.uv] exclude-newer = "7 days"` means every `uv lock` re-resolves against a
sliding cutoff, so an unrelated relock will pick up newer releases.
...
- Testing:
  ...
  - Enterprise unit tests live under `enterprise/tests`; run them from the repo root with `uv run pytest enterprise/tests/unit/test_xxx.py` (`pytest.ini` puts `enterprise/` on `sys.path`)
...
**Prerequisites:**
- Python 3.12
- uv >= 0.11 (for dependency management)
- Node.js 22.x (for frontend)
- Docker (optional)

**Setup Steps:**
1. Build the project: `make build`. That is the whole setup — enterprise dependencies are
   part of the root `pyproject.toml`, so there is nothing extra to install under `enterprise/`.
2. Pre-commit hooks are installed by `make build`; `make lint-enterprise` runs the enterprise config.

**Running Enterprise Tests:**
```bash
# Enterprise unit tests (full suite), from the repo root
uv run pytest --forked -n auto -s ./enterprise/tests/unit --cov=enterprise --cov-branch

# Test specific modules (faster for development)
uv run pytest enterprise/tests/unit/storage --confcutdir=enterprise/tests/unit/storage

# Enterprise linting (IMPORTANT: use --show-diff-on-failure to match GitHub CI)
uv run pre-commit run --all-files --show-diff-on-failure --config ./enterprise/dev_config/python/.pre-commit-config.yaml
```

**Running Enterprise Server:**
```bash
make start-backend  # Development mode with hot reload (saas_server:app)
# or
make run  # Full application (backend + frontend)
```

**Key Configuration Files:**
- `pyproject.toml` / `uv.lock` - Dependencies for the whole repo, enterprise included
- `Makefile` - Build and run commands; `make start-backend` starts the enterprise server
- `enterprise/dev_config/python/` - Linting and type checking configuration
- `enterprise/migrations/` - Database migration files (run alembic from `enterprise/`)
...
- Run every command from the repo root; `enterprise/` has no build or environment of its own
...
- If enterprise imports fail outside pytest, put `enterprise/` on the path: `PYTHONPATH=enterprise uv run ...`
