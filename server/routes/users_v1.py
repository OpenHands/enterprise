...
async def get_current_user_saas(
    ...
    authenticated_user_id = await user_context.get_user_id()
    super_role = (
        await authorization.get_user_super_role(authenticated_user_id)
        if authenticated_user_id is not None
        else None
    )
    global_permissions: list[JsonValue] = []
    if super_role:
        global_permissions.extend(
            sorted(
                permission.value
    ...
    user_info_data['global_permissions'] = global_permissions

    profile_user_id = await user_context.get_user_id()
    if profile_user_id is None:
        raise HTTPException(status.HTTP_401_UNAUTHORIZED, detail='Not authenticated')
    profile_metadata = await get_auth_services().accounts.get_profile_metadata(
        profile_user_id
    )
    if profile_metadata is not None:
        user_info_data.update(_USER_WIRE.validate_python(profile_metadata))
