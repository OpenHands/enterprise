...
@native_auth_router.post('/password/change', status_code=204)
async def change_password(
    ...
    result = await get_native_auth_service().change_password(
...
@native_auth_router.post('/password/reset/complete', status_code=204)
async def complete_password_reset(body: ResetBody, request: Request) -> None:
    await get_native_auth_service().complete_password_reset(
...
@native_auth_router.post('/enrollment/accept-membership', status_code=204)
async def accept_membership(
    body: TokenBody, user_id: str | None = Depends(get_user_id)
) -> None:
    await get_native_auth_service().accept_membership(
