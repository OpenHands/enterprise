...
from server.auth.native_csrf import issue_csrf
...
async def csrf(request: Request, response: Response) -> dict[str, str]:
    ...
    if session_token and await service.authenticate_session(session_token) is None:
        await clear_session_cookie(response, request)
        session_token = None
    return {'csrf_token': issue_csrf(request, response, session_token)}
