...
def validate_native_ancillary_config() -> None:
    ...
    if os.getenv('ENABLE_LINEAR', 'false').lower() in ('true', '1'):
        raise ValueError(
            'Linear account linking is unavailable without Keycloak; disable the Linear integration'
        )
    ...
    if slack_enabled or any(
        ...
        raise ValueError(
            'Slack account linking is unavailable without Keycloak; disable the Slack integration'
        )
