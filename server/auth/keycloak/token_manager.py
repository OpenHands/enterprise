...
from server.auth.keycloak.errors import (
    is_transient_keycloak_error,
    translate_keycloak_errors,
)
from server.auth.keycloak.manager import get_keycloak_admin, get_keycloak_openid
...
class TokenManager(ProviderTokenRefresher):
    ...
    async def get_keycloak_tokens(
        ...
        try:
            ...
            if is_transient_keycloak_error(exc):
                raise
            logger.warning('Keycloak token exchange failed')
            return None, None
    ...
    @translate_keycloak_errors
    async def get_idp_token_by_user_id(self, user_id: str, idp: ProviderType) -> str:
        from server.auth.provider_credentials import ProviderCredentialService

        token = await ProviderCredentialService(self._jwt_svc).get_token(user_id, idp)
    ...
    async def get_idp_token_from_offline_token(
        ...
        try:
            ...
            if is_transient_keycloak_error(e):
                raise
            error_message = str(e)
            if 'invalid_grant' in error_message or 'session not found' in error_message:
                logger.warning('Keycloak offline session expired or invalid')
    ...
    async def get_idp_token_from_idp_user_id(
        ...
        token = await ProviderCredentialService(self._jwt_svc).token_for_actor(
            idp, str(idp_user_id)
        )
    ...
    async def get_user_id_from_idp_user_id(
        ...
        user_id = await ProviderCredentialService(self._jwt_svc).resolve_user(
            idp, str(idp_user_id)
        )
        ...
        for query_params in wildcard_queries:
            try:
                ...
                if is_transient_keycloak_error(e):
                    raise
                logger.debug(
                    'Keycloak wildcard query failed',
                    extra={'query_kind': next(iter(query_params))},
                )
    ...
    async def unlink_idp(self, user_id: str, idp: ProviderType) -> None:
        ...
        await ProviderCredentialService(self._jwt_svc).disconnect(user_id, idp)
    ...
    async def refresh(self, refresh_token: str) -> dict:
        try:
            ...
            logger.info('keycloak_refresh_failed')
            raise
    ...
    async def logout(self, refresh_token: str):
        try:
            ...
            logger.warning('Keycloak logout failed')
            raise
