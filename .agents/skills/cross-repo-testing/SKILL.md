...
**CI guard:** `check-package-versions.yml` blocks merge to `main` if `[project] dependencies` or `[tool.uv.sources]` pins a package to a git ref. Before the OpenHands PR can merge, the SDK PR must be merged and released to PyPI, then the pin must be replaced with the released version number.
...
| **SDK pin blocks merge** | `check-package-versions.yml` prevents merging an OpenHands PR that pins a package to a git ref in `[project] dependencies` or `[tool.uv.sources]`. The SDK must be released to PyPI first. |
