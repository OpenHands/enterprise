...
```bash
curl -sS "$OPENHANDS_BASE_URL/api/admin/models?provider=openhands&limit=100" \
...
```bash
curl -X POST "$OPENHANDS_BASE_URL/api/admin/models" \
  -H "Authorization: Bearer $OPENHANDS_ADMIN_TOKEN" \
...
```bash
curl -X PUT "$OPENHANDS_BASE_URL/api/admin/models/openhands/glm-5.2" \
  -H "Authorization: Bearer $OPENHANDS_ADMIN_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"is_enabled": false}'
```
...
```bash
curl -X DELETE "$OPENHANDS_BASE_URL/api/admin/models/openhands/glm-5.2" \
  -H "Authorization: Bearer $OPENHANDS_ADMIN_TOKEN"
```
