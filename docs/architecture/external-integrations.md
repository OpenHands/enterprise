...
|-----------|---------|----------|
| **Integration Routes** | Webhook endpoints per service | `server/routes/integration/` |
| **Integration Managers** | Business logic per service | `integrations/{service}/` |
| **Token Manager** | Store/retrieve OAuth tokens | `server/auth/token_manager.py` |
| **Callback Processor** | Handle agent → service updates | `integrations/{service}/*_callback_processor.py` |
